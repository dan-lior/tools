...
std::pair<Matrix<3>, Vector<3>> Miscellaneous::read_from_mmo(const std::string& filename)
{
    Vector<3> translation;
    Vector<3> rotation; 
    Vector<3> scale;

    assert(Miscellaneous::validate_filename_extension(filename, "mmo"));
...

    {
        ...
        std::getline(file, line);
        std::stringstream ss(line);
        Vector<3> rotation_in_degrees; 
        ss >> rotation_in_degrees(0) >> rotation_in_degrees(1) >> rotation_in_degrees(2); 
        rotation = M_PI/180 * rotation_in_degrees;
    }
    ...
    std::cerr << "translation: " << translation.transpose() << std::endl;
    std::cerr << "rotation: " << rotation.transpose() << std::endl;
    std::cerr << "scale: " << scale.transpose() << std::endl;

    Matrix<3> Rx = MiscellaneousMath::rotation(Vector<3>(1,0,0), rotation(0));
    Matrix<3> Ry = MiscellaneousMath::rotation(Vector<3>(0,1,0), rotation(1));
    Matrix<3> Rz = MiscellaneousMath::rotation(Vector<3>(0,0,1), rotation(2));

    // todo: determine the convention in mesh mixer for the order of these rotations
    
 //   return std::make_pair(Rx*Ry*Rz,translation);
    return std::make_pair(Rz*Ry*Rx,translation);
}
